WITH extracted_info AS (
  SELECT
    ...
    CASE
        ...
        WHEN REGEXP_CONTAINS(text,r'(?i).*fault.*|.*overhead.*') then "Equipment Fault"
        ...
        WHEN REGEXP_CONTAINS(text,r'(?i).*operation.*incident.*|.*operation.*issue.*') then "Operational Incident"
        ...
        WHEN REGEXP_CONTAINS(text,r'(?i).*storm.*') then "Storm Damage"
        WHEN REGEXP_CONTAINS(text,r'(?i).*extreme.*weather.*') then "Storm Damage"
        ...
        WHEN REGEXP_CONTAINS(text,r'(?i).*game.*|.*concert.*|.*#AFL.*|.*match.*|.*day out.*|.*flemington racecourse.*|.*cricket.*') then "Event"
